...
router.post('/addDna', async function (req, res) {
    let [data, fields] = await DB.DBQuery('SELECT * FROM `dna_list` WHERE dna=?', [req.body.dna]);
    if (data.length == 0) {
        await DB.DBQuery('INSERT INTO `dna_list` (`id`, `dna`) VALUES(?, ?)', [req.body.id, req.body.dna]);
    }
    res.send({
        success: true
    });
...
router.get('/dnaList', async function (req, res) {
    let [data, fields] = await DB.DBQuery('SELECT * FROM dna_list');
    res.send({
        dnaList: data
    });
})
